CREATE OR REPLACE PACKAGE pkg_info_admin6 IS
  ...
  ***************************************************************************************************/
  ...
  -- 获取个人签名印章
  FUNCTION f_getseal(i_adminuri VARCHAR2) RETURN CLOB;
END;
/
CREATE OR REPLACE PACKAGE BODY pkg_info_admin6 IS
  -- 查询列表
  PROCEDURE p_getlist
  (
    i_forminfo IN CLOB, -- 表单信息(前台请求)
  ...
  BEGIN
    ...
    -- 验证用户权限
    pkg_qp_verify.p_check('MD912', i_operuri, i_opername, o_code, o_msg);
    IF o_code <> 'EC00' THEN
      RETURN;
    END IF;
    ...
    v_sql := mystring.f_concat(v_sql, ' WHERE E1.admintype= ''MT06''');
  ...
  BEGIN
    ...
    INSERT INTO info_admin (adminuri, adminname, admintype, sort, operuid, operunm, linktel) VALUES (i_useruri, i_username, 'MT06', i_sort, i_operuri, i_opername, i_linktel);
  ...
  BEGIN
    ...
    UPDATE info_admin t
       SET t.sort = i_sort, t.operuid = i_operuri, t.operunm = i_opername, t.modifieddate = SYSDATE
  ...
  ***************************************************************************************************/
  ...
  BEGIN
    ...
    IF v_type = '1' THEN
      ...
      SELECT json_value(i_forminfo, '$.i_sign' RETURNING VARCHAR2(32767)) INTO v_sign FROM dual;
      pkg_info_admin6.p_ins(v_useruri, v_username, v_password, v_linktel, v_sort, v_sign, i_operuri, i_opername, o_code, o_msg);
      IF o_code <> 'EC00' THEN
        RETURN;
      END IF;
    ELSIF v_type = '0' THEN
      ...
      BEGIN
        ...
        WHILE v_i <= 100 LOOP
          ...
          IF mystring.f_isnull(v_useruri) THEN
            v_i := 100;
          ELSE
            pkg_info_admin6.p_del(v_useruri, i_operuri, i_opername, v_code, v_msg);
            IF v_code <> 'EC00' THEN
              v_num := v_num + 1;
              IF v_num > 1 THEN
    ...
    ELSIF v_type = '2' THEN
      ...
      SELECT json_value(i_forminfo, '$.i_sort') INTO v_sort FROM dual;
      SELECT json_value(i_forminfo, '$.i_sign' RETURNING VARCHAR2(32767)) INTO v_sign FROM dual;
      pkg_info_admin6.p_upd(v_useruri, v_username, v_linktel, v_sort, v_sign, i_operuri, i_opername, o_code, o_msg);
      IF o_code <> 'EC00' THEN
        RETURN;
      END IF;
    ELSIF v_type = '4' THEN
      SELECT json_value(i_forminfo, '$.i_useruri') INTO v_useruri FROM dual;
      SELECT json_value(i_forminfo, '$.i_password') INTO v_password FROM dual;
      pkg_info_admin6.p_upd_passwd(v_useruri, v_password, i_operuri, i_opername, o_code, o_msg);
      IF o_code <> 'EC00' THEN
        RETURN;
      END IF;
    END IF;
  ...
  -- 获取个人签名印章
  FUNCTION f_getseal(i_adminuri VARCHAR2) RETURN CLOB AS
    v_result CLOB;
  BEGIN
    SELECT t.signseal INTO v_result FROM info_admin_sign t WHERE t.adminuri = i_adminuri;
    RETURN v_result;
  EXCEPTION
    WHEN OTHERS THEN
      RETURN '';
