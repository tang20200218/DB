CREATE OR REPLACE PACKAGE pkg_info_org IS
  ...
  ***************************************************************************************************/
  ...
  (
    ...
    i_comname  IN VARCHAR2, -- 单位名称
    i_comid    IN VARCHAR2, -- 单位空间号
    i_comcode  IN VARCHAR2, -- 机构代码
    i_sort     IN VARCHAR2, -- 排序
    i_rs       IN VARCHAR2, -- 向空间发送数据的路由
    i_operuri  IN VARCHAR2, -- 操作人标识
    i_opername IN VARCHAR2, -- 操作人姓名
    o_id       OUT VARCHAR2, -- 单位标识
    o_code     OUT VARCHAR2, -- 操作结果:错误码
    o_msg      OUT VARCHAR2 -- 成功/错误原因
  );
...
END;
/
CREATE OR REPLACE PACKAGE BODY pkg_info_org IS

  /***************************************************************************************************
  名称     : pkg_info_org.p_getlist
  功能描述 : 查询列表
  ...
  <RESPONSE>
    <ROWS>
      <ROW row_id="序号">
        <id>唯一标识</id>
        <comname>单位名称</comname>
        <comid>单位空间号</comid>
        <comcode>机构代码</comcode>
        <sort>排序号</sort>
  ...
  ***************************************************************************************************/
  ...
  BEGIN
    ...
    BEGIN
      IF mystring.f_isnotnull(v_conditions) THEN
        ...
        SELECT myxml.f_getvalue(v_xml, '/cs/comname') INTO v_cs_comname FROM dual;
    ...
    IF mystring.f_isnotnull(v_cs_comname) THEN
      v_sql := mystring.f_concat(v_sql, ' AND instr(E1.objname, ''', v_cs_comname, ''') > 0');
    END IF;
    ...
    BEGIN
      ...
      LOOP
        ...
        SELECT objid, objname, objcode, fromtype, status, errmsg, opername, modifieddate
          INTO v_row_comid, v_row_comname, v_row_comcode, v_row_fromtype, v_row_status, v_row_errmsg, v_row_opername, v_row_createddate
        ...
        v_row := mystring.f_concat(v_row, ',"comcode":"', v_row_comcode, '"');
  ...
  /***************************************************************************************************
  名称     : pkg_info_org.p_getlist2
  功能描述 : 查询列表(打印、导出时查询)
  
  修订记录：
  版本号  编辑时间    编辑人  修改描述
  1.0.0   2023-06-13  唐金鑫  创建
  ...
  BEGIN
    ...
    BEGIN
      ...
      LOOP
        ...
        v_row := mystring.f_concat(v_row, '<comid>', v_row_comid, '</comid>');
        v_row := mystring.f_concat(v_row, '<comname>', myxml.f_escape(v_row_comname), '</comname>');
        v_row := mystring.f_concat(v_row, '<comcode>', v_row_comcode, '</comcode>');
  ...
  ***************************************************************************************************/
  ...
  BEGIN
    ...
    SELECT json_value(i_forminfo, '$.i_comcode') INTO v_comcode FROM dual;
    mydebug.wlog('v_comcode', v_comcode);
  ...
  BEGIN
    ...
    mydebug.wlog('i_comcode', i_comcode);
    ...
    IF mystring.f_isnull(i_comname) THEN
      o_code := 'EC02';
      o_msg  := '单位名称为空,请检查！';
      mydebug.wlog(3, o_code, o_msg);
      RETURN;
    ...
    IF mystring.f_isnull(i_comcode) THEN
      o_code := 'EC02';
      o_msg  := '机构代码为空,请检查！';
      mydebug.wlog(3, o_code, o_msg);
      RETURN;
    ...
    IF mystring.f_isnull(v_id) THEN
      ...
      VALUES
        (v_id, i_comid, i_comname, i_comcode, 1, v_sort, i_kindid, v_kindidpath, 1, v_status, i_operuri, i_opername);
    ELSE
      UPDATE info_register_obj t
         SET t.objname      = i_comname,
             t.kindid       = i_kindid,
    ...
    -- 存储路由信息
    IF mystring.f_isnotnull(i_comid) AND mystring.f_isnotnull(i_rs) THEN
      pkg_exch_to_site.p_ins(i_comid, i_comname, 'QT10', i_rs, o_code, o_msg);
      IF o_code <> 'EC00' THEN
        ROLLBACK;
        RETURN;
      END IF;
  ...
  ***************************************************************************************************/
  ...
  BEGIN
    ...
    IF v_type = '1' THEN
      ...
      SELECT json_value(i_forminfo, '$.i_kindid') INTO v_kindid FROM dual;
      SELECT json_value(i_forminfo, '$.i_comname') INTO v_comname FROM dual;
      SELECT json_value(i_forminfo, '$.i_comid') INTO v_comid FROM dual;
      SELECT json_value(i_forminfo, '$.i_comcode') INTO v_comcode FROM dual;
      SELECT json_value(i_forminfo, '$.i_sort') INTO v_sort FROM dual;
      SELECT json_value(i_forminfo, '$.i_rs') INTO v_rs FROM dual;
      pkg_info_org.p_ins(v_kindid, v_comname, v_comid, v_comcode, v_sort, v_rs, i_operuri, i_opername, v_id, o_code, o_msg);
      IF o_code <> 'EC00' THEN
        RETURN;
      END IF;
      ...
      BEGIN
        ...
        WHILE v_i <= 100 LOOP
          ...
          IF mystring.f_isnull(v_id) THEN
            v_i := 100;
          ELSE
            pkg_info_org.p_del(v_id, i_operuri, i_opername, o_code, o_msg);
            IF v_code <> 'EC00' THEN
              v_num := v_num + 1;
              IF v_num > 1 THEN
    ...
    ELSIF v_type = '2' THEN
      ...
      SELECT json_value(i_forminfo, '$.i_id') INTO v_id FROM dual;
      SELECT json_value(i_forminfo, '$.i_sort') INTO v_sort FROM dual;
      pkg_info_org.p_upd(v_id, v_sort, i_operuri, i_opername, o_code, o_msg);
      IF o_code <> 'EC00' THEN
        RETURN;
      END IF;
    END IF;
      ...
      WHILE v_i <= 9999 LOOP
        ...
        ELSE
          ...
          IF mystring.f_isnull(v_id) THEN
            ...
            VALUES
              (v_id, v_name, v_code, 1, v_sort, v_kindid, v_kindidpath, 2, 0, i_operuri, i_opername);
          
            DELETE FROM info_register_queue WHERE id = v_code;
            INSERT INTO info_register_queue (id, datatype) VALUES (v_code, 1);
